...
begin
   Put_Line ("X0: " & X0.JSON_Image);
   X0 := "changed";
   Put_Line ("X0: " & X0.JSON_Image);
   X0 := 1;
   Put_Line ("X0: " & X0.JSON_Image);

   Put_Line ("X1: " & X1.JSON_Image);
   Put_Line ("X2: " & X2.JSON_Image);
   Put_Line ("X3: " & X3.JSON_Image);
   Put_Line ("X4: " & X4.JSON_Image);

   Put_Line ("M5: " & M5.JSON_Image);
   Put_Line ("V6: " & V6.JSON_Image);

   Put_Line ("Crate: " & Alire_Crate.Crate.JSON_Image);

   Put_Line ("Map indexing: M4 (""one"") => " & M4 ("one").JSON_Image);
   Put_Line ("Map nested indexing: Crate (""depends-on"") (""aaa"") => "
             & Alire_Crate.Crate ("depends-on").As_Map ("aaa").JSON_Image);

   Put_Line ("Map nested indexing alt syntax: "
             & M5 ("two")("two").JSON_Image);

   Put_Line ("Map nested indexing alt alt syntax: " & M5 ("two" / "two").JSON_Image);

   Put_Line ("Nested mixed indexing alt: " & M6 ("zri")(2)("one").JSON_Image);
   Put_Line ("Map nested indexing w vec: " & M6 (Yeison.Vec'["zri", 2]).JSON_Image);
   Put_Line ("Nested mixed indexing alt: " & M6 ("zri" / 2 / "one").JSON_Image);

   Put_Line ("Vec indexing: V6 (1) = " & V6 (1).JSON_Image);
   Put_Line ("Vec nested indexing V6 (2) (2) = "
             & V6 (2).As_Vec (2).JSON_Image);
   Put_Line ("Vec nested indexing alt syntax V6 ((2, 2)) = " & V6 (2)(2).JSON_Image);
   Put_Line ("Vec mixed indexing: " & V6.Get (3 / "one").JSON_Image);

   Put_Line ("Real image: " & A4.JSON_Image);
end Demo_JSON;
