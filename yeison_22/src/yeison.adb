...
package body Yeison is
   ...
   --  These replace Yeison_12's public Make.* scalar constructors: clients use
   --  literals (1, 3.14, "text") instead, so the builders need not be exposed.

   function New_Nil return Any
   is (Any_Parent with Impl => Nil_Impl);

   function New_Bool (V : Boolean) return Any
   is (Any_Parent with Impl => new Any_Impl'(Bool_Kind, (Bool_Kind, V)));

   function New_Int (V : Big_Int) return Any
   is (Any_Parent with Impl => new Any_Impl'(Int_Kind, (Int_Kind, V)));

   function New_Real (V : Reals.General_Real) return Any
   is (Any_Parent with Impl => new Any_Impl'(Real_Kind, (Real_Kind, V)));

   function New_Str (V : Text) return Any
   is (Any_Parent with Impl => new Any_Impl'(Str_Kind, (Str_Kind, U (V))));

   function New_Scalar (This : Scalar) return Any
   is (case This.Data.Kind is
          ...
          when Str_Kind  => New_Str  (S (This.Data.Str)));
   ...
   is (New_Real (Reals.New_Real (Big_Real'Value (Img))));
   ...
   function Nil   return Any is (New_Nil);
   function True  return Any is (New_Bool (Standard.True));
   function False return Any is (New_Bool (Standard.False));
   ...
   is (New_Scalar (This).Image (Format));
   ...
   --  In-place, any-keyed insertion. Internal (the public any-keyed insertion
   --  is the functional Insert, or M (Key) := Value through indexing).

   procedure Set (This    : in out Any;
                  Key     : Any;
                  Value   : Any;
                  Replace : Boolean := False)
   is
            ...
            case Target.Kind is
               when Map_Kind =>
                  if not Target.Impl.Map.Contains (Pos) then
                     if Create then
                        Insert_Impl (Target.Impl.all, Pos, New_Nil);
                     else
                  ...
                  begin
                     if Index <= 0 then
                        ...
                        if Index > Len + 1 then
                           ...
                           Target.Impl.Vec.Append (New_Nil);
                        end if;
